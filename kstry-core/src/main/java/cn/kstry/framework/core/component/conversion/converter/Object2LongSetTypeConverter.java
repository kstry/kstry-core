...
public class Object2LongSetTypeConverter implements TypeConverter<Object, Set> {
    ...
    public Set<?> doConvert(Object source, @Nullable Class<?> needType) {
        if (source instanceof String) {
            return Sets.newHashSet(JSON.parseArray((String) source, Long.class));
        }
        if (source instanceof Iterable || source.getClass().isArray()) {
            return Sets.newHashSet(JSON.parseArray(JSON.toJSONString(source), Long.class));
        }
    ...
    public String getConvertName() {
        return TypeConverterNames.OBJECT_TO_LONG_SET;
    }
}
