        ...
        try {
            ...
            this.enhancedDataOperator = new EnhancedDataOperator() {
                ...
                public <T> Optional<T> getResult(String typeConverter) {
                    ...
                    return Optional.ofNullable(typeConverterProcessor.convert(typeConverter, getResult()).getValue()).map(t -> (T) t);
                }
                ...
                public <T> Optional<T> getReqData(String name, String typeConverter) {
                    ...
                    return Optional.ofNullable(typeConverterProcessor.convert(typeConverter, getReqData(name)).getValue()).map(t -> (T) t);
                }
                ...
                public <T> Optional<T> getStaData(String name, String typeConverter) {
                    ...
                    return Optional.ofNullable(typeConverterProcessor.convert(typeConverter, getStaData(name)).getValue()).map(t -> (T) t);
                }
                ...
                public <T> Optional<T> getVarData(String name, String typeConverter) {
                    ...
                    return Optional.ofNullable(typeConverterProcessor.convert(typeConverter, getVarData(name)).getValue()).map(t -> (T) t);
                }
                ...
                public <T> Optional<T> getData(String expression, String typeConverter) {
                    ...
                    return Optional.ofNullable(typeConverterProcessor.convert(typeConverter, getData(expression)).getValue()).map(t -> (T) t);
                }
                ...
                public <T> Optional<T> getTaskProperty(String typeConverter) {
                    ...
                    return Optional.ofNullable(typeConverterProcessor.convert(typeConverter, getTaskProperty()).getValue()).map(t -> (T) t);
                }
                ...
                public <T> Optional<T> iterDataItem(String typeConverter) {
                    ...
                    return Optional.ofNullable(typeConverterProcessor.convert(typeConverter, iterDataItem()).getValue()).map(t -> (T) t);
                }
                ...
                public <T> Optional<T> getResult(Class<T> targetClass) {
                    ...
                    return Optional.ofNullable(typeConverterProcessor.convert(getResult(), targetClass).getValue());
                }
                ...
                public <T> Optional<T> getReqData(String name, Class<T> targetClass) {
                    ...
                    return Optional.ofNullable(typeConverterProcessor.convert(getReqData(name), targetClass).getValue());
                }
                ...
                public <T> Optional<T> getStaData(String name, Class<T> targetClass) {
                    ...
                    return Optional.ofNullable(typeConverterProcessor.convert(getStaData(name), targetClass)).map(t -> (T) t);
                }
                ...
                public <T> Optional<T> getVarData(String name, Class<T> targetClass) {
                    ...
                    return Optional.ofNullable(typeConverterProcessor.convert(getVarData(name), targetClass)).map(t -> (T) t);
                }
                ...
                public <T> Optional<T> getData(String expression, Class<T> targetClass) {
                    ...
                    return Optional.ofNullable(typeConverterProcessor.convert(getData(expression), targetClass)).map(t -> (T) t);
                }
                ...
                public <T> Optional<T> getTaskProperty(Class<T> targetClass) {
                    ...
                    return Optional.ofNullable(typeConverterProcessor.convert(getTaskProperty(), targetClass)).map(t -> (T) t);
                }
                ...
                public <T> Optional<T> iterDataItem(Class<T> targetClass) {
                    ...
                    return Optional.ofNullable(typeConverterProcessor.convert(iterDataItem(), targetClass)).map(t -> (T) t);
                }
                ...
                public boolean setResult(Object target) {
                    ...
                    try {
                        ...
                        OperatorStoryBus.this.returnResult = target;
                        InvokeMethodThreadLocal.getServiceTask().ifPresent(element ->
                                monitorTracking.trackingNodeNotice(element, () -> NoticeTracking.build(null, null, ScopeTypeEnum.RESULT, target, target.getClass(), null))
                        );
                        return true;
                    } finally {
                        wLock.unlock();
                ...
                private boolean doSetData(String name, ScopeData scopeData, Object target) {
                    ...
                    try {
                        ...
                        InvokeMethodThreadLocal.getServiceTask().filter(s -> setRes).ifPresent(element ->
                                monitorTracking.trackingNodeNotice(element, () -> NoticeTracking.build(null, name, scopeData.getScopeDataEnum(), target, targetClass, null))
                        );
                        return setRes;
                    } finally {
                        wLock.unlock();
