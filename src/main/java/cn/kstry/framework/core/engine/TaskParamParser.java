...
public class TaskParamParser {
    ...
    public Object[] parseParams(boolean tracking, IterDataItem<?> iterDataItem, ServiceTask serviceTask,
        ...
        fillTaskParams(tracking, storyBus, serviceTask, params, serviceTask.getTaskParams(), paramInjectDefs, storyBus.getScopeDataOperator());
    ...
    private void fillTaskParams(boolean tracking, StoryBus storyBus, ServiceTask serviceTask,
                                Object[] params, Map<String, Object> taskParams, List<ParamInjectDef> paramInjectDefs, ScopeDataOperator scopeDataOperator) {
        ...
        Map<String, String> converterMap = getConverterMap(taskParams);
        ...
        for (int i = 0; i < paramInjectDefs.size(); i++) {
            ...
            try {
                if (Map.class.isAssignableFrom(iDef.getParamType()) && iDef.getParamType().isAssignableFrom(val.getClass())) {
                    Pair<String, ?> convertPair = typeConverterProcessor.convert(converterMap.get(fName), val, iDef.getParamType());
                ...
                if (!(val instanceof Map)) {
                    Pair<String, Object> paramPair = parseParamValue(converterMap.get(fName), val, scopeDataOperator, iDef.getParamType());
                ...
                Map<String, String> fieldConverterMap = getConverterMap(valMap);
                for (Field field : params[i].getClass().getDeclaredFields()) {
                    ...
                    Pair<String, Object> fieldPair = parseParamValue(fieldConverterMap.get(targetName), v, scopeDataOperator, field.getType());
    ...
    private Map<String, String> getConverterMap(Map<String, Object> valMap) {
        Map<String, String> fieldConverterMap = Maps.newHashMap();
        valMap.keySet().forEach(k -> {
            Pair<String, String> mapping = TaskServiceUtil.getConverterMapping(k);
            if (StringUtils.isAnyBlank(mapping.getKey(), mapping.getValue())) {
                return;
            }
            fieldConverterMap.put(mapping.getKey(), mapping.getValue());
            valMap.put(mapping.getKey(), valMap.get(k));
            valMap.remove(k);
        });
    ...
    @SuppressWarnings("unchecked")
    private Pair<String, Object> parseParamValue(String converterName, Object val, ScopeDataOperator scopeDataOperator, Class<?> type) {
        if (val instanceof String) {
            String valStr = (String) val;
            if (valStr.startsWith("@") && ElementParserUtil.isValidDataExpression(valStr.substring(1))) {
                val = scopeDataOperator.getData(valStr.substring(1)).orElse(type.isPrimitive() ? ElementParserUtil.initPrimitive(type) : null);
            }
        }
        if (val == null) {
            return ImmutablePair.of(null, type.isPrimitive() ? ElementParserUtil.initPrimitive(type) : null);
        }
        Pair<String, Object> convertPair = (Pair<String, Object>) engineModule.getTypeConverterProcessor().convert(converterName, val, type);
