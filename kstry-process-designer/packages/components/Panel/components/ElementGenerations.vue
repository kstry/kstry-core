<template>
  <el-collapse-item name="base-info">
    <template #title>
      <collapse-title :title="isProcess || isSubProcess ? '流程信息' : '节点信息'">
        <lucide-icon name="Info" />
      </collapse-title>
    </template>

    <edit-item label="ID">
      <el-input v-model="elementId" maxlength="200" @change="updateElementId" />
    </edit-item>

    <edit-item label="Name">
      <el-input v-model="elementName" maxlength="200" @change="updateElementName" />
    </edit-item>
...
export default {
  ...
  methods: {
    reloadGenerationData() {
      ...
      this.isSubProcess = !!getActive() && getActive().type === "bpmn:SubProcess";
